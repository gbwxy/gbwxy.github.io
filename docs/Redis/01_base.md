...
![try-redis](./images/try.redis.io.png)
...
![Redis 发布订阅 (pub/sub) 功能](./images/redis-pub-sub.png)
...
![pub/sub 实现消息队列演示](./images/redis-pubsub-message-queue.png)
...
![img](./images/image-20220720194154133.png)
...
![文件事件处理器（file event handler）](./images/redis-event-handler.png)
...
![redis4.0 more thread](./images/redis4.0-more-thread.png)
...
![Redis 过期字典](./images/redis-expired-dictionary.png)
...
![redis.conf 对于 hz 的注释](./images/redis.conf-hz.png)
...
![Redis 事务](./images/redis-transactions.png)
...
![Redis 为什么不支持回滚](./images/redis-rollback.png)
...
![](./images/redis-pipeline-vs-transaction.png)
...
![bigkey 判定标准](./images/bigkey-criterion.png)
...
![阿里云Key分析](./images/aliyun-key-analysis.png)
...
![京东零售开源的 hotkey](./images/jd-hotkey.png)
...
![通过阿里云的Proxy Query Cache优化热点Key问题](./images/aliyun-hotkey-proxy-query-cache.png)
...
![缓存穿透](./images/redis-cache-penetration.png)
...
![Bloom Filter 的简单原理示意图](./images/bloom-filter-simple-schematic-diagram.png)

Bloom Filter 会使用一个较大的 bit 数组来保存所有的数据，数组中的每个元素都只占用 1 bit ，并且每个元素只能是 0 或者 1（代表 false 或者 true），这也是 Bloom Filter 节省内存的核心所在。这样来算的话，申请一个 100w 个元素的位数组只占用 1000000Bit / 8 = 125000 Byte = 125000/1024 KB ≈ 122KB 的空间。

![位数组](./images/bloom-filter-bit-table.png)

具体是这样做的：把所有可能存在的请求的值都存放在布隆过滤器中，当用户请求过来，先判断用户发来的请求的值是否存在于布隆过滤器中。不存在的话，直接返回请求参数错误信息给客户端，存在的话才会走下面的流程。

加入布隆过滤器之后的缓存处理流程图如下：

![加入布隆过滤器之后的缓存处理流程图](./images/redis-cache-penetration-bloom-filter.png)
...
![缓存击穿](./images/redis-cache-breakdown.png)
...
![缓存雪崩](./images/redis-cache-avalanche.png)
